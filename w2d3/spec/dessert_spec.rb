...
describe Dessert do
  ...
  # only 3 instance variables in initialize
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("cake2", "lots", chef) }.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Flay the Great Baker")
      expect(cake.serve).to eq("Chef Flay the Great Baker has made 10 cakes!")
    end
  end
